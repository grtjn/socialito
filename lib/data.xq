...
declare function data:get (
    ...
    as element()*
{
    let $log := fn:trace(<log time="{date:current-time()}">Start data:get({$collection},{$user},{$keys})</log>, "DEBUG")
    let $data :=
        if (not($keys)) then
            data:get-all($collection, $user)
    ...
    let $log := fn:trace(<log time="{date:current-time()}">End data:get({$collection},{$user},{$keys})</log>, "DEBUG")
    return
        $data
};
...
declare function data:get-all (
    ...
    as element()*
{
    let $log := fn:trace(<log time="{date:current-time()}">Start data:get-all({$collection},{$user})</log>, "DEBUG")
    let $data :=
    ...
    let $log := fn:trace(<log time="{date:current-time()}">End data:get-all({$collection},{$user})</log>, "DEBUG")
    return
        $data
};
...
declare function data:get-paged (
    ...
    as element()*
{
    let $log := fn:trace(<log time="{date:current-time()}">Start data:get-paged({$collection},{$user},{$start},{$end})</log>, "DEBUG")
    let $data :=
        if ($collection eq $data:hashtags-collection) then
            (:
